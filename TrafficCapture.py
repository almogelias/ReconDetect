...
def createFolder(writepath):
    ...
    fileNameWithTime = fileName + '_' + str(int(datetime.datetime.now().timestamp())) + '.txt'  # Get File with Unix time
    try:
        os.mkdir(writepath)
    except OSError:
...
def get_networkAdapter():
    ...
    for intface, addr_list in addresses.items():
        ...
        if intface in stats and getattr(stats[intface], "isup"): #Get all the adapter which is up
            available_networks.append(Adapter(intface,addr_list[1].address,addr_list[1].netmask)) #Write Adapter info from adapterInfo.py
...
### Calling Sniffer Driver (Scapy Lib)
   ...
   #sniff(iface=onlineAdapter.name, prn=pkt_callback,filter=filter) #Sniff All !
